...
class Unet_pp(nn.Module):
    ...
    def forward(self, x_img):
        ...
        out_conv_00_crop = CenterCrop(size = out_upconv_01.size(dim=0))(torch.permute(out_conv_00, (2,0,1)))
        out_conv_00_crop = torch.permute(out_conv_00_crop, (1,2,0))
        out_concat_01 = torch.cat((out_conv_00_crop, out_upconv_01), dim = 2)
        out_conv_01 = self.conv_01(out_concat_01)
        out_conv_01 = self.activation(out_conv_01)

        out_upconv_11 = self.upconv_11(out_conv_20)
        out_conv_10_crop = CenterCrop(size = out_upconv_11.size(dim=0))(torch.permute(out_conv_10, (2,0,1)))
        out_conv_10_crop = torch.permute(out_conv_10_crop, (1,2,0))
        out_concat_11 = torch.cat((out_conv_10_crop, out_upconv_11), dim = 2)
        out_conv_11 = self.conv_11(out_concat_11)
        out_conv_11 = self.activation(out_conv_11)

        out_upconv_21 = self.upconv_21(out_conv_30)
        out_conv_20_crop = CenterCrop(size = out_upconv_21.size(dim=0))(torch.permute(out_conv_20, (2,0,1)))
        out_conv_20_crop = torch.permute(out_conv_20_crop, (1,2,0))
        out_concat_21 = torch.cat((out_conv_20_crop, out_upconv_21), dim = 2)
        out_conv_21 = self.conv_21(out_concat_21)
        out_conv_21 = self.activation(out_conv_21)

        out_upconv_31 = self.upconv_31(out_conv_40)
        out_conv_30_crop = CenterCrop(size = out_upconv_31.size(dim=0))(torch.permute(out_conv_30, (2,0,1)))
        out_conv_30_crop = torch.permute(out_conv_30_crop, (1,2,0))
        out_concat_31 = torch.cat((out_conv_30_crop, out_upconv_31), dim = 2)
        out_conv_31 = self.conv_31(out_concat_31)
        out_conv_31 = self.activation(out_conv_31)


        out_upconv_02 = self.upconv_02(out_conv_11)
        out_conv_00_crop = CenterCrop(size = out_upconv_02.size(dim=0))(torch.permute(out_conv_00, (2,0,1)))
        out_conv_00_crop = torch.permute(out_conv_00_crop, (1,2,0))
        out_conv_01_crop = CenterCrop(size = out_upconv_02.size(dim=0))(torch.permute(out_conv_01, (2,0,1)))
        out_conv_01_crop = torch.permute(out_conv_01_crop, (1,2,0))
        out_concat_02 = torch.cat((out_conv_00_crop, out_conv_01_crop, out_upconv_02), dim = 2)
        out_conv_02 = self.conv_02(out_concat_02)
        out_conv_02 = self.activation(out_conv_02)

        out_upconv_12 = self.upconv_12(out_conv_21)
        out_conv_10_crop = CenterCrop(size = out_upconv_12.size(dim=0))(torch.permute(out_conv_10, (2,0,1)))
        out_conv_10_crop = torch.permute(out_conv_10_crop, (1,2,0))
        out_conv_11_crop = CenterCrop(size = out_upconv_12.size(dim=0))(torch.permute(out_conv_11, (2,0,1)))
        out_conv_11_crop = torch.permute(out_conv_11_crop, (1,2,0))
        out_concat_12 = torch.cat((out_conv_10_crop, out_conv_11_crop, out_upconv_12), dim = 2)
        out_conv_12 = self.conv_12(out_concat_12)
        out_conv_12 = self.activation(out_conv_12)

        out_upconv_22 = self.upconv_22(out_conv_31)
        out_conv_20_crop = CenterCrop(size = out_upconv_22.size(dim=0))(torch.permute(out_conv_20, (2,0,1)))
        out_conv_20_crop = torch.permute(out_conv_20_crop, (1,2,0))
        out_conv_21_crop = CenterCrop(size = out_upconv_22.size(dim=0))(torch.permute(out_conv_21, (2,0,1)))
        out_conv_21_crop = torch.permute(out_conv_21_crop, (1,2,0))
        out_concat_22 = torch.cat((out_conv_20_crop, out_conv_21_crop, out_upconv_22), dim = 2)
        out_conv_22 = self.conv_22(out_concat_22)
        out_conv_22 = self.activation(out_conv_22)


        out_upconv_03 = self.upconv_03(out_conv_12)
        out_conv_00_crop = CenterCrop(size = out_upconv_03.size(dim=0))(torch.permute(out_conv_00, (2,0,1)))
        out_conv_00_crop = torch.permute(out_conv_00_crop, (1,2,0))
        out_conv_01_crop = CenterCrop(size = out_upconv_03.size(dim=0))(torch.permute(out_conv_01, (2,0,1)))
        out_conv_01_crop = torch.permute(out_conv_01_crop, (1,2,0))
        out_conv_02_crop = CenterCrop(size = out_upconv_03.size(dim=0))(torch.permute(out_conv_02, (2,0,1)))
        out_conv_02_crop = torch.permute(out_conv_02_crop, (1,2,0))
        out_concat_03 = torch.cat((out_conv_00_crop, out_conv_01_crop, out_conv_02_crop, out_upconv_03), dim = 2)
        out_conv_03 = self.conv_03(out_concat_03)
        out_conv_03 = self.activation(out_conv_03)

        out_upconv_13 = self.upconv_13(out_conv_22)
        out_conv_10_crop = CenterCrop(size = out_upconv_13.size(dim=0))(torch.permute(out_conv_10, (2,0,1)))
        out_conv_10_crop = torch.permute(out_conv_10_crop, (1,2,0))
        out_conv_11_crop = CenterCrop(size = out_upconv_13.size(dim=0))(torch.permute(out_conv_11, (2,0,1)))
        out_conv_11_crop = torch.permute(out_conv_11_crop, (1,2,0))
        out_conv_12_crop = CenterCrop(size = out_upconv_13.size(dim=0))(torch.permute(out_conv_12, (2,0,1)))
        out_conv_12_crop = torch.permute(out_conv_12_crop, (1,2,0))
        out_concat_13 = torch.cat((out_conv_10_crop, out_conv_11_crop, out_conv_12_crop, out_upconv_13), dim = 2)
        out_conv_13 = self.conv_13(out_concat_13)
        out_conv_13 = self.activation(out_conv_13)


        out_upconv_04 = self.upconv_04(out_conv_13)
        out_conv_00_crop = CenterCrop(size = out_upconv_13.size(dim=0))(torch.permute(out_conv_00, (2,0,1)))
        out_conv_00_crop = torch.permute(out_conv_00_crop, (1,2,0))
        out_conv_01_crop = CenterCrop(size = out_upconv_13.size(dim=0))(torch.permute(out_conv_01, (2,0,1)))
        out_conv_01_crop = torch.permute(out_conv_01_crop, (1,2,0))
        out_conv_02_crop = CenterCrop(size = out_upconv_13.size(dim=0))(torch.permute(out_conv_02, (2,0,1)))
        out_conv_02_crop = torch.permute(out_conv_02_crop, (1,2,0))
        out_conv_03_crop = CenterCrop(size = out_upconv_13.size(dim=0))(torch.permute(out_conv_03, (2,0,1)))
        out_conv_03_crop = torch.permute(out_conv_03_crop, (1,2,0))
        out_concat_04 = torch.cat((out_conv_00_crop, out_conv_01_crop, out_conv_02_crop, out_conv_03_crop, out_upconv_04), dim = 2)
